...
from .linear_gradient_filtering import wrap_linear_layer
from .linear_lora import wrap_linear_lora_layer
# from .pointwise_conv_wht import wrap_conv_layer
from .pointwise_conv_lora import wrap_pointwise_conv_lora_layer
from .pointwise_conv_gradient_filtering import wrap_conv_layer
...
def add_grad_filter(module: nn.Module, cfg):
    if cfg["type"] == "linear_wht":
        module = wrap_linear_layer(
            module, cfg["radius"], cfg["activate"]
        )
        ...
        module.mlp.fc1 = wrap_conv_layer(
            module.mlp.fc1, cfg["radius"], cfg["activate"]
        )
        module.mlp.fc2 = wrap_conv_layer(
            module.mlp.fc2, cfg["radius"], cfg["activate"]
        )
        ...
        module.token_mixer.qkv = wrap_linear_layer(
            module.token_mixer.qkv, cfg["radius"], cfg["activate"]
        )
        module.token_mixer.proj = wrap_linear_layer(
            module.token_mixer.proj, cfg["radius"], cfg["activate"]
        )
        module.mlp.fc1 = wrap_linear_layer(
            module.mlp.fc1, cfg["radius"], cfg["activate"]
        )
        module.mlp.fc2 = wrap_linear_layer(
            module.mlp.fc2, cfg["radius"], cfg["activate"]
        )
        ...
        module.token_mixer.proj[1] = wrap_conv_layer(
            module.token_mixer.proj[1], cfg["radius"], cfg["activate"]
        )
        module.token_mixer.q[0] = wrap_conv_layer(
            module.token_mixer.q[0], cfg["radius"], cfg["activate"]
        )
        module.token_mixer.k[0] = wrap_conv_layer(
            module.token_mixer.k[0], cfg["radius"], cfg["activate"]
        )
        module.token_mixer.v[0] = wrap_conv_layer(
            module.token_mixer.v[0], cfg["radius"], cfg["activate"]
        )
        module.token_mixer.talking_head1 = wrap_conv_layer(
            module.token_mixer.talking_head1,
            cfg["radius"],
            cfg["activate"]
        )
        module.token_mixer.talking_head2 = wrap_conv_layer(
        ...
        module.conv[0].conv = wrap_conv_layer(
            module.conv[0].conv, cfg["radius"], cfg["activate"]
        )
        module.conv[2].conv = wrap_conv_layer(
            module.conv[2].conv, cfg["radius"], cfg["activate"]
        )
    elif cfg["type"] == "swinv2blk_attn":
        module.attn.w_msa.qkv = wrap_linear_layer(
            module.attn.w_msa.qkv, cfg["radius"], cfg["activate"]
        )
        module.attn.w_msa.proj = wrap_linear_layer(
            module.attn.w_msa.proj, cfg["radius"], cfg["activate"]
        )
        module.ffn.layers[0][0] = wrap_linear_layer(
            module.ffn.layers[0][0], cfg["radius"], cfg["activate"]
        )
        module.ffn.layers[1] = wrap_linear_layer(
            module.ffn.layers[1], cfg["radius"], cfg["activate"]
        )
